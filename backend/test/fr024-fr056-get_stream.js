...
describe('fr024-fr056-get_stream', () =>{
...
it('Correct expected error when no value for filterType with filters.js', ()=> {
  ...
  it('Get stream for game with content.js', ()=> {
    ...
    .catch((error) => {
      expect(error.description).to.equal("no streams found - check spelling of gameId");
    });
});
...
it('Correct expected error when no value for filterValue with content.js', ()=> {
  ...
  .catch((error) => {
    expect(error.description).to.equal("bad request - no gameId is given");
  });
});
...
it('Correct expected error when no game_id is found for streams with content.js', ()=> {
  let ok = true;
  const inputs = {
    assetType: 'streams', filterType: 'contextual', filterBy: 'gameId'
  }
  ...
  .catch((error) => {
    expect(error.description).to.equal("bad request - no gameId found");
  });
});
...
it('Correct expected error when no game_id is found for games with content.js', ()=> {
  let ok = true;
  const inputs = {
    assetType: 'games', filterType: 'contextual', filterBy: 'topGames', quantity: 50
  }
...
it('Correct expected error when requesting more than 100 streams with content.js', ()=> {
  let ok = true;
  const inputs = {
    assetType: 'streams', filterType: 'contextual', filterBy: 'gameId', gameId: '57690', quantity: 101
  }
...
it('Correct expected error when for no streams exist for chosen game_id with filters.js', ()=> {
  let ok = true;
  const inputs = {
    assetType: 'streams', filterType: 'contextual', filterBy: 'gameId', gameId: '57690', quantity: 10
  }
...
it('Correct expected error when incorrect filter_by with content.js', ()=> {
  let ok = true;
  const inputs = {
    assetType: 'streams', filterType: 'contextual', filterBy: 'not_game_id'
  }
